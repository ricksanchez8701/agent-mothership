...
#define ERRLBL(x) printf("  -> ret=%ld errno=%d %s\n", (long)r, errno, errno?strerror(errno):"ok")

int main(void) {
    /* 7. memfd_create + execveat fileless */
    int fd = syscall(__NR_memfd_create, "anon", 0);
    printf("[7] memfd_create          ret=%d errno=%d %s\n", fd, errno, fd<0?strerror(errno):"");
    if (fd >= 0) {
        write(fd, "#!/bin/sh\necho FILELESS-OK\nexit 0\n", 34);
        lseek(fd, 0, SEEK_SET);
        errno = 0;
        ...
        long r = syscall(__NR_execveat, fd, "", argv, envp, AT_EMPTY_PATH);
        printf("   execveat(memfd)       ret=%ld errno=%d %s\n", r, errno, errno?strerror(errno):"ok");
        /* if ENOENT, try execveat with a fake name but that needs path */
    }
    if (fd >= 0) close(fd);

    /* 8. O_TMPFILE + linkat(AT_EMPTY_PATH) */
    ...
    if (tf >= 0) {
        ...
        errno = 0;
        int r = linkat(AT_FDCWD, pbuf, AT_FDCWD, "/tmp/door/materialized", AT_SYMLINK_FOLLOW);
        printf("   linkat /proc/self/fd  ret=%d errno=%d %s\n", r, errno, errno?strerror(errno):"ok");
        if (r == 0) {
            printf("   file created: %s\n", access("/tmp/door/materialized", F_OK)==0?"YES":"no");
            unlink("/tmp/door/materialized");
        }
        errno = 0;
    ...
    errno = 0; system("printf '#!/bin/echo\\nhi\\n' > /tmp/door/s.sh; chmod +x /tmp/door/s.sh");
    char *argv[] = {"s.sh", NULL};
    char *envp[] = {NULL};
    long r = syscall(__NR_execve, "/tmp/door/s.sh", argv, envp);
    ...
    /* execveat(AT_FDCWD, absolute path, AT_SYMLINK_NOFOLLOW) */
    errno = 0;
